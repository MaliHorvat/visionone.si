...
export function ServiceImageSplit({ imageSrc, imageAlt, children, reverse = false, priority = false }: Props) {
  return (
    <div className="grid gap-0 overflow-hidden rounded-2xl border border-[var(--vo-border)] bg-[var(--vo-surface)] md:grid-cols-2">
      <div
        className={`relative aspect-[16/10] min-h-[220px] w-full min-w-0 overflow-hidden bg-[var(--vo-surface-2)] md:min-h-[min(48vh,360px)] ${
          reverse ? "md:order-2" : ""
        }`}
      >
        <Image
